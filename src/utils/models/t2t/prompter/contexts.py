...
You will only respond with your own message and shouldn't include the time stamp or the other person's response.
You shall never break from the character described in the remainder of this system prompt:
""".format(
    ...
    def __init__(self, jaison):
        ...
        if jaison.config.t2t_enable_context_twitch_chat:
            self.event_listener = self.EventListener(
                self, jaison.twitch.broadcast_server
            )
    ...
    def add_history(self, time, name, message):
        self.convo_history.append({"time": time, "name": name, "message": message})
    ...
    def commit_history(self):
        self.convo_history = self.convo_history[
            -(self.jaison.config.t2t_convo_retention_length or 20) :
        ]
        self.uncommited_history_len = 0
    ...
    def reload_name_translations(self):
        with open(
            os.path.join(
                self.jaison.config.t2t_name_translation_dir,
                self.jaison.config.t2t_name_translation_file,
            ),
            "r",
        ) as f:
    ...
    def msg_o_to_line(self, o):
        time = "[{}]".format(o["time"])
        name = (
            "<{}>".format(self.SELF_IDENTIFIER)
            if o["name"] == self.SELF_IDENTIFIER
            else "[{}]".format(self.translate_name(o["name"]))
        )
        message = o["message"]
